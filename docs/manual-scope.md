...
- explicit task-to-task deps via `CoreTaskArgs.set_dependencies(deps, count)`
...
- `set_dependencies(...)` may be called at most once per CoreTaskArgs with `count > 0`;
...
- plus explicit fanins from `CoreTaskArgs.set_dependencies(...)`
...
- same submit-time explicit dependency model through `CoreTaskArgs.set_dependencies(...)`
