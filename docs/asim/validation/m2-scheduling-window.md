# M2 campaign — the GroupQueue against scan_and_claim (2026-09-04)

What the GroupQueue was worth when M0 was `scan_and_claim` and the gate was the
**scheduling window** (`sched_cost=`), not `device_wall`. Method and fidelity:
[../validation.md](../validation.md). The later campaign measures a different
metric against a different baseline and its figures are not comparable with
these: [m2-group-queue.md](m2-group-queue.md).
