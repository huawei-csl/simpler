...
PullRequest
  ...
  ├── packaging-matrix       (ubuntu + macOS)        — [needs !examples_only]
  ├── ut                     (ubuntu + macOS)        — Python + C++ UT, no hardware [needs ut_affected]
  ├── detect-changes         (ubuntu-latest)         — outputs non_code_only, a{2a3,5}_changed, {st,ut}_affected
