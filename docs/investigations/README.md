...
- [2026-07 — Replacing the tmr ready-task queue with a VNQueues concurrent queue](2026-07-vnq-ready-queue-swap.md) — four VNQ swaps A/B'd against the Vyukov ready queue. Root cause: `device_wall` is 95-99% `graph_build` (orchestration), scheduler is off the critical path, so swaps are neutral on all but the narrowest dispatch-heavy workloads. The atomic-element+reindex variant is a **real but workload-specific** win — decode_fwd 40L **−0.50%** (paired n=11, p≈1e-8) yet decode_tq 40L **+0.19%** (slower) and DSv4 ~0, so not landable unconditionally. Mailbox batched-push is a no-op (no workload has `cond_count>1`); ticket-lock is neutral; SPCMT/SPCMD N/A (the `PTO2LocalReadyBuffer` donation path is dead code, only 3 sched threads)
