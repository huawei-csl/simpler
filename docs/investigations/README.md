...
- [2026-07 — Alternating half-core polling/dispatch in the AICPU scheduler loop](2026-07-aicpu-scheduler-half-split-polling.md) — measured & reverted: mixed, no clear signal (4 improved / 3 regressed / 1 noise of 8 examples on Effective, all `|Δ| < 1.3%`), no pattern found separating the two sets
- [2026-07 — Can independent AICPU compute overlap a pending COND LDR?](2026-07-aicpu-ldr-compute-overlap.md) — measured: yes, partially, even without reordering; a pipelined fetch/process ordering captures more of it (~30-38 ns/iteration once local compute ≥ ~50-100 ns) but saves nothing at the scheduler's current few-ns per-core workload — reconsider if that workload grows
