...
- [2026-09 — Where qwen3's wall clock actually goes (and two scheduler theories it kills)](2026-09-qwen3-scheduler-headroom.md) — characterization: the AICPU schedulers are ~100% saturated while AICore lanes idle 1,860 ms against 1,251 ms busy, and 73% of that idle follows the 40 full-chip MIX tasks whose consumer chain collapses to 24 cube cores + 26 single-core tasks, leaving 48 AIV cores structurally unfillable. Falsifies cube pipelining (raised 48.4%→71.3%, wall unchanged) and boundary dispatch latency as causes; immediate dispatch is a scan-avoidance mechanism whose payoff scales with wavefront width
- [2026-09 — scan_and_claim: speculative gated staging of guaranteed-ready consumers](2026-09-sac-speculative-gated-staging.md) — implemented & correct but measured as a regression (qwen3 +4.8%, PA +0.9–1.6%) and reverted: `fanin_remaining == 1` behind a running producer is an exact readiness guarantee, but gating defers the AICore ACK so staging pre-commits cores, and the scheduling freedom given up outweighs the dispatch latency bought. Carries the progressive-submit staleness trap (any new by-id slot read needs the `current_task_index` bound)
