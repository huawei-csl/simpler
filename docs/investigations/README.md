...
- [2026-08 — hbg: per-block Graph Definitions and cross-layer reuse](2026-08-hbg-graph-block-decomposition.md) — dropped: splitting DeepSeek-V4's one 743-node layer-pair Definition into four block Definitions (and replaying two of them at the peeled last layer) is a **regression on `main` as of #1897**, because `graph_begin` holds one recording slot and silently demotes a Graph whose key differs from the in-flight recording's — 79 of 82 intended submissions recorded and host tasks rose 1131 → 1486. Measured before #1897 the same form gave 1131 → 835 tasks and 4.31 → 3.15 ms, so the verdict flips if the slot stops demoting. Keeps the structural map that made the reuse provable (367 kernels → 169 classes / 132 by code alone; which blocks can share a Definition and why the hash-routed MoE cannot), the two indices and the last layer's different `hc_post` destination, and the arithmetic that a recorded node costs about what a submitted task costs so break-even sits near three occurrences
