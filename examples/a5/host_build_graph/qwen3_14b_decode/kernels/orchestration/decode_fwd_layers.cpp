...
class ScratchArena {
public:
    explicit ScratchArena(const ChipTensor &storage) :
        storage_(storage) {}

    ChipTensor allocate(const TensorCreateInfo &create_info) {
    ...
    SIMPLER_SCOPE() {
        ...
        for (uint32_t layer = 0; layer < num_layers; layer += 1) {
            SIMPLER_SCOPE() {
                ...
                ChipTensor q_norm_w_inline124 = layer_view(ext_q_norm_weight, layer, 1);
                ChipTensor k_norm_w_inline114 = layer_view(ext_k_norm_weight, layer, 1);
                ChipTensor k_cache = layer_view(ext_k_cache, layer, cache_rows);
                ChipTensor v_cache = layer_view(ext_v_cache, layer, cache_rows);
                ChipTensor wo = layer_view(ext_wo, layer, 5120);
                ChipTensor w_gate = layer_view(ext_w_gate, layer, 5120);
                ChipTensor w_up = layer_view(ext_w_up, layer, 5120);
                ChipTensor w_down = layer_view(ext_w_down, layer, 17408);
                ChipTensor post_rms_weight = layer_view(ext_post_rms_weight, layer, 1);
                ...
                auto layer_definition = [](const GraphTaskArgs &args) {
                    ...
                    const ChipTensor &next_normed = args.tensor(3).ref();
                    const ChipTensor &ext_input_rms_weight = args.tensor(4).ref();
                    const ChipTensor &ext_wq = args.tensor(5).ref();
                    const ChipTensor &ext_wk = args.tensor(6).ref();
                    const ChipTensor &ext_wv = args.tensor(7).ref();
                    const ChipTensor &q_norm_w_inline124 = args.tensor(8).ref();
                    const ChipTensor &k_norm_w_inline114 = args.tensor(9).ref();
                    const ChipTensor &ext_seq_lens = args.tensor(10).ref();
                    const ChipTensor &ext_block_table = args.tensor(11).ref();
                    const ChipTensor &ext_slot_mapping = args.tensor(12).ref();
                    const ChipTensor &ext_rope_cos = args.tensor(13).ref();
                    const ChipTensor &ext_rope_sin = args.tensor(14).ref();
                    const ChipTensor &ext_k_cache = args.tensor(15).ref();
                    const ChipTensor &ext_v_cache = args.tensor(16).ref();
                    const ChipTensor &ext_wo = args.tensor(17).ref();
                    const ChipTensor &ext_w_gate = args.tensor(18).ref();
                    const ChipTensor &ext_w_up = args.tensor(19).ref();
                    const ChipTensor &ext_w_down = args.tensor(20).ref();
                    const ChipTensor &ext_post_rms_weight = args.tensor(21).ref();
                    const ChipTensor &pa_workspace = args.tensor(22).ref();
                    const ChipTensor &pa_metadata = args.tensor(23).ref();
                    const ChipTensor &bf16_scratch = args.tensor(24).ref();
                    const ChipTensor &fp32_scratch = args.tensor(25).ref();
                    ...
                    ChipTensor inv_rms_states_inline176 = fp32_arena.allocate(inv_rms_states_inline176_ci);
                    ChipTensor q_proj_inline139 = fp32_arena.allocate(q_proj_inline139_ci);
                    ChipTensor k_proj_inline135 = fp32_arena.allocate(k_proj_inline135_ci);
                    ChipTensor v_proj_inline255 = fp32_arena.allocate(v_proj_inline255_ci);
                    ChipTensor q_tnd_flat_inline127 = bf16_arena.allocate(q_tnd_flat_inline127_ci);
                    ChipTensor attn_out_inline282 = bf16_arena.allocate(attn_out_inline282_ci);
                    ChipTensor down_acc_all_inline168 = fp32_arena.allocate(down_acc_all_inline168_ci);
                    ChipTensor gate_acc_all_inline203 = fp32_arena.allocate(gate_acc_all_inline203_ci);
                    ChipTensor up_acc_all_inline303 = fp32_arena.allocate(up_acc_all_inline303_ci);
                    ChipTensor attn_proj_fp32_inline220 = fp32_arena.allocate(attn_proj_fp32_inline220_ci);
                    ChipTensor post_norm_partial_inline118 = fp32_arena.allocate(post_norm_partial_inline118_ci);
                    ChipTensor mlp_norm_in_inline71 = bf16_arena.allocate(mlp_norm_in_inline71_ci);
