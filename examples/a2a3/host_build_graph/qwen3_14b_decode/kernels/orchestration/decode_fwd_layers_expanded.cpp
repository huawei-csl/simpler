...
constexpr uint64_t kScratchAlignmentBytes = 1024;
constexpr uint32_t kBfloat16ScratchElements = 524288;
constexpr uint32_t kFloat32ScratchElements = 918016;

class ScratchArena {
public:
    explicit ScratchArena(const simpler::hbg::Tensor &storage) :
        storage_(storage) {}

    simpler::hbg::Tensor allocate(const TensorCreateInfo &create_info) {
        always_assert(create_info.dtype == storage_.dtype);
        const uint64_t element_size = get_element_size(storage_.dtype);
        const uint64_t alignment = kScratchAlignmentBytes / element_size;
        cursor_ = (cursor_ + alignment - 1) / alignment * alignment;
        const uint64_t tensor_elements = create_info.buffer_size_bytes() / element_size;
        const uint64_t aligned_elements = (tensor_elements + alignment - 1) / alignment * alignment;
        always_assert((cursor_ + aligned_elements) * element_size <= storage_.buffer.size);

        simpler::hbg::Tensor tensor;
        init_tensor_from_create_info(
            tensor, create_info, reinterpret_cast<void *>(static_cast<uintptr_t>(storage_.buffer.addr)),
            storage_.buffer.size
        );
        tensor.owner_task_id = storage_.owner_task_id;
        tensor.start_offset = storage_.start_offset + cursor_;
        cursor_ += aligned_elements;
        return tensor;
    }

private:
    const simpler::hbg::Tensor &storage_;
    uint64_t cursor_{0};
};
...
__attribute__((visibility("default"))) void aicpu_orchestration_entry(const ChipTaskArgs &orch_args) {
    ...
    const simpler::hbg::Tensor &ext_wq = orch_args.tensor(2).ref();
    const simpler::hbg::Tensor &ext_wk = orch_args.tensor(3).ref();
    const simpler::hbg::Tensor &ext_wv = orch_args.tensor(4).ref();
    const simpler::hbg::Tensor &ext_q_norm_weight = orch_args.tensor(5).ref();
    const simpler::hbg::Tensor &ext_k_norm_weight = orch_args.tensor(6).ref();
    const simpler::hbg::Tensor &ext_seq_lens = orch_args.tensor(7).ref();
    const simpler::hbg::Tensor &ext_block_table = orch_args.tensor(8).ref();
    const simpler::hbg::Tensor &ext_slot_mapping = orch_args.tensor(9).ref();
    const simpler::hbg::Tensor &ext_rope_cos = orch_args.tensor(10).ref();
    const simpler::hbg::Tensor &ext_rope_sin = orch_args.tensor(11).ref();
    const simpler::hbg::Tensor &ext_k_cache = orch_args.tensor(12).ref();
    const simpler::hbg::Tensor &ext_v_cache = orch_args.tensor(13).ref();
    const simpler::hbg::Tensor &ext_wo = orch_args.tensor(14).ref();
    const simpler::hbg::Tensor &ext_w_gate = orch_args.tensor(15).ref();
    const simpler::hbg::Tensor &ext_w_up = orch_args.tensor(16).ref();
    const simpler::hbg::Tensor &ext_w_down = orch_args.tensor(17).ref();
    const simpler::hbg::Tensor &ext_post_rms_weight = orch_args.tensor(18).ref();
    const simpler::hbg::Tensor &ext_out = orch_args.tensor(19).ref();
    ...
    SIMPLER_SCOPE() {
        ...
        const simpler::hbg::Tensor &pa_metadata = alloc_0.get_ref(0);
        const simpler::hbg::Tensor &pa_workspace = alloc_0.get_ref(1);
        const simpler::hbg::Tensor &cur = alloc_0.get_ref(2);
        const simpler::hbg::Tensor &normed = alloc_0.get_ref(3);
        ...
        simpler::hbg::Tensor cur__rv_v7 = cur;
        simpler::hbg::Tensor normed__rv_v5 = normed;
        uint32_t bf16_scratch_shapes[1] = {kBfloat16ScratchElements};
        TensorCreateInfo bf16_scratch_ci(bf16_scratch_shapes, 1, DataType::BFLOAT16);
        uint32_t fp32_scratch_shapes[1] = {kFloat32ScratchElements};
        TensorCreateInfo fp32_scratch_ci(fp32_scratch_shapes, 1, DataType::FLOAT32);
        ...
        for (uint32_t layer = 0; layer < num_layers; layer += 1) {
            SIMPLER_SCOPE() {
                ...
                const simpler::hbg::Tensor &next_hidden = layer_storage.get_ref(storage_base);
                const simpler::hbg::Tensor &next_normed = layer_storage.get_ref(storage_base + 1);
                const simpler::hbg::Tensor &bf16_scratch = layer_storage.get_ref(4);
                const simpler::hbg::Tensor &fp32_scratch = layer_storage.get_ref(5);
                auto layer_view = [](const simpler::hbg::Tensor &tensor, uint32_t layer_index, uint32_t rows) {
                ...
                simpler::hbg::Tensor next_input_rms_weight =
                    layer_view(ext_input_rms_weight, std::min<uint32_t>(layer + 1, num_layers - 1), 1);
                simpler::hbg::Tensor wq = layer_view(ext_wq, layer, 5120);
                simpler::hbg::Tensor wk = layer_view(ext_wk, layer, 5120);
                simpler::hbg::Tensor wv = layer_view(ext_wv, layer, 5120);
                simpler::hbg::Tensor q_norm_w_inline124 = layer_view(ext_q_norm_weight, layer, 1);
                simpler::hbg::Tensor k_norm_w_inline114 = layer_view(ext_k_norm_weight, layer, 1);
                simpler::hbg::Tensor k_cache = layer_view(ext_k_cache, layer, cache_rows);
                simpler::hbg::Tensor v_cache = layer_view(ext_v_cache, layer, cache_rows);
                simpler::hbg::Tensor wo = layer_view(ext_wo, layer, 5120);
                simpler::hbg::Tensor w_gate = layer_view(ext_w_gate, layer, 5120);
                simpler::hbg::Tensor w_up = layer_view(ext_w_up, layer, 5120);
                simpler::hbg::Tensor w_down = layer_view(ext_w_down, layer, 17408);
                simpler::hbg::Tensor post_rms_weight = layer_view(ext_post_rms_weight, layer, 1);
                simpler::hbg::Tensor graph_normed = normed__rv_v5;
                ...
                graph_args.add_inout(next_normed);
                graph_args.add_input(next_input_rms_weight);
                graph_args.add_input(wq);
                graph_args.add_input(wk);
                graph_args.add_input(wv);
                graph_args.add_input(q_norm_w_inline124);
                graph_args.add_input(k_norm_w_inline114);
                graph_args.add_input(ext_seq_lens);
                graph_args.add_input(ext_block_table);
                graph_args.add_input(ext_slot_mapping);
                graph_args.add_input(ext_rope_cos);
                graph_args.add_input(ext_rope_sin);
                graph_args.add_inout(k_cache);
                graph_args.add_inout(v_cache);
                graph_args.add_input(wo);
                graph_args.add_input(w_gate);
                graph_args.add_input(w_up);
                graph_args.add_input(w_down);
                graph_args.add_input(post_rms_weight);
                graph_args.add_inout(pa_workspace);
                graph_args.add_inout(pa_metadata);
                graph_args.add_inout(bf16_scratch);
                graph_args.add_inout(fp32_scratch);

                auto layer_definition = [](const GraphTaskArgs &args) {
                    const simpler::hbg::Tensor &cur__rv_v7 = args.tensor(0).ref();
                    const simpler::hbg::Tensor &normed__rv_v5 = args.tensor(1).ref();
                    const simpler::hbg::Tensor &next_hidden = args.tensor(2).ref();
                    const simpler::hbg::Tensor &next_normed = args.tensor(3).ref();
                    const simpler::hbg::Tensor &ext_input_rms_weight = args.tensor(4).ref();
                    const simpler::hbg::Tensor &ext_wq = args.tensor(5).ref();
                    const simpler::hbg::Tensor &ext_wk = args.tensor(6).ref();
                    const simpler::hbg::Tensor &ext_wv = args.tensor(7).ref();
                    const simpler::hbg::Tensor &q_norm_w_inline124 = args.tensor(8).ref();
                    const simpler::hbg::Tensor &k_norm_w_inline114 = args.tensor(9).ref();
                    const simpler::hbg::Tensor &ext_seq_lens = args.tensor(10).ref();
                    const simpler::hbg::Tensor &ext_block_table = args.tensor(11).ref();
                    const simpler::hbg::Tensor &ext_slot_mapping = args.tensor(12).ref();
                    const simpler::hbg::Tensor &ext_rope_cos = args.tensor(13).ref();
                    const simpler::hbg::Tensor &ext_rope_sin = args.tensor(14).ref();
                    const simpler::hbg::Tensor &ext_k_cache = args.tensor(15).ref();
                    const simpler::hbg::Tensor &ext_v_cache = args.tensor(16).ref();
                    const simpler::hbg::Tensor &ext_wo = args.tensor(17).ref();
                    const simpler::hbg::Tensor &ext_w_gate = args.tensor(18).ref();
                    const simpler::hbg::Tensor &ext_w_up = args.tensor(19).ref();
                    const simpler::hbg::Tensor &ext_w_down = args.tensor(20).ref();
                    const simpler::hbg::Tensor &ext_post_rms_weight = args.tensor(21).ref();
                    const simpler::hbg::Tensor &pa_workspace = args.tensor(22).ref();
                    const simpler::hbg::Tensor &pa_metadata = args.tensor(23).ref();
                    const simpler::hbg::Tensor &bf16_scratch = args.tensor(24).ref();
                    const simpler::hbg::Tensor &fp32_scratch = args.tensor(25).ref();
                    ScratchArena bf16_arena(bf16_scratch);
                    ScratchArena fp32_arena(fp32_scratch);

                    constexpr int64_t i = 0;
                    constexpr int64_t next_gamma_idx = 0;
                    ...
                    TensorCreateInfo inv_rms_states_inline176_ci(
                        inv_rms_states_inline176_ci_shapes, 2, DataType::FLOAT32
                    );
                    ...
                    TensorCreateInfo attn_proj_fp32_inline220_ci(
                        attn_proj_fp32_inline220_ci_shapes, 2, DataType::FLOAT32
                    );
                    ...
                    simpler::hbg::Tensor inv_rms_states_inline176 = fp32_arena.allocate(inv_rms_states_inline176_ci);
                    simpler::hbg::Tensor q_proj_inline139 = fp32_arena.allocate(q_proj_inline139_ci);
                    simpler::hbg::Tensor k_proj_inline135 = fp32_arena.allocate(k_proj_inline135_ci);
                    simpler::hbg::Tensor v_proj_inline255 = fp32_arena.allocate(v_proj_inline255_ci);
                    simpler::hbg::Tensor q_tnd_flat_inline127 = bf16_arena.allocate(q_tnd_flat_inline127_ci);
                    simpler::hbg::Tensor attn_out_inline282 = bf16_arena.allocate(attn_out_inline282_ci);
                    simpler::hbg::Tensor down_acc_all_inline168 = fp32_arena.allocate(down_acc_all_inline168_ci);
                    simpler::hbg::Tensor gate_acc_all_inline203 = fp32_arena.allocate(gate_acc_all_inline203_ci);
                    simpler::hbg::Tensor up_acc_all_inline303 = fp32_arena.allocate(up_acc_all_inline303_ci);
                    simpler::hbg::Tensor attn_proj_fp32_inline220 = fp32_arena.allocate(attn_proj_fp32_inline220_ci);
                    simpler::hbg::Tensor post_norm_partial_inline118 =
                        fp32_arena.allocate(post_norm_partial_inline118_ci);
                    simpler::hbg::Tensor mlp_norm_in_inline71 = bf16_arena.allocate(mlp_norm_in_inline71_ci);
                    simpler::hbg::Tensor inv_rms_tile_inline126 = fp32_arena.allocate(inv_rms_tile_inline126_ci);
                    simpler::hbg::Tensor mlp_tile_inline149 = bf16_arena.allocate(mlp_tile_inline149_ci);
                    ...
                    SIMPLER_SCOPE(ScopeMode::MANUAL) {
                        ...
                        simpler::hbg::Tensor q_tnd_inline191 = q_tnd_flat_inline127.reshape(q_tnd_inline191_shapes, 3);
                        uint32_t attn_out_tnd_inline79_shapes[3] = {16, 40, 128};
                        simpler::hbg::Tensor attn_out_tnd_inline79 =
                        ...
                        const simpler::hbg::Tensor &attn_out_tnd_inline79__ssa_v1 = attn_out_tnd_inline79;
                        TaskId attn_done_tid_inline78 = task_11_outs.task_id();
                        uint32_t attn_out_inline282__ssa_v4_shapes[2] = {16, 5120};
                        simpler::hbg::Tensor attn_out_inline282__ssa_v4 =
                ...
                // Expanded rather than recorded: the definition runs here, so every
                // task it submits enters the graph directly instead of arriving as one
                // GRAPH task the device Scheduler expands. Same tasks, same edges --
                // what changes is that the whole graph is visible to the manager, and
                // to dep-gen, at submit time. rt_submit_graph takes this same path
                // itself whenever the boundary args are not cacheable.
                layer_definition(graph_args);
                simpler::hbg::Tensor cur__ssa_v8 = next_hidden;
                simpler::hbg::Tensor normed__ssa_v6 = next_normed;
