...
This is a completion/smoke case: upstream pypto-lib runs the same fixture with
``golden_fn=None`` (component-level golden checks live with the standalone
kernels there), and a full-network torch reference does not exist in either
repo. The case validates that the harvested distributed program — 368 incore
kernels plus a 7.8k-line chip orchestration per rank — compiles, dispatches
across both dies, drives the comm-window protocol to completion, and terminates
cleanly.

**The parameters live in child memory.** Each of the 78 per-rank parameters is
one ``Worker.alloc_child_tensor`` buffer on its own rank, allocated once before
the first round and reused by every round: the runtime passes a device-resident
tensor through without malloc, H2D staging or copy-back. What goes into those
buffers is the one knob: by default the harvest's fixture is streamed in one
parameter at a time (42.6 GiB per rank, from
``simpler_setup.goldens.deepseek_v4_flash_decode.param_tensors``), while
``--skip-golden`` uploads nothing and leaves them uninitialized — which is what
makes the case cheap enough to measure and to run repeatedly. There is no golden
either way, so the pytest wrapper beside this file passes ``skip_golden=True``:
...
from simpler_setup.goldens.deepseek_v4_flash_decode import (
    N_RANKS,
    PARAM_SPEC_BY_NAME,
    T,
    param_tensors,
...
CASE_LABEL = "DeepseekV4FlashDecode"
...
#   ("param",  name, argtype)               -> this rank's shard  (child-memory tensor)
...
_ARG_STEPS: list[tuple] = [
    ("param", "embed_weight", "INPUT"),
    ("param", "hc_attn_fn", "INPUT"),
    ("param", "hc_attn_scale", "INPUT"),
    ("param", "hc_attn_base", "INPUT"),
    ("param", "attn_norm_w", "INPUT"),
    ("param", "wq_a", "INPUT"),
    ("param", "wq_b", "INPUT"),
    ("param", "wq_b_scale", "INPUT"),
    ("param", "wkv", "INPUT"),
    ("param", "gamma_cq", "INPUT"),
    ("param", "gamma_ckv", "INPUT"),
    ("param", "kv_cache", "INOUT"),
    ("param", "attn_sink", "INPUT"),
    ("param", "wo_a", "INPUT"),
    ("param", "wo_b", "INPUT"),
    ("param", "wo_b_scale", "INPUT"),
    ("param", "hca_cmp_wkv", "INPUT"),
    ("param", "hca_cmp_wgate", "INPUT"),
    ("param", "hca_cmp_ape", "INPUT"),
    ("param", "hca_cmp_norm_w", "INPUT"),
    ("param", "hca_compress_state", "INOUT"),
    ("param", "csa_cmp_wkv", "INPUT"),
    ("param", "csa_cmp_wgate", "INPUT"),
    ("param", "csa_cmp_ape", "INPUT"),
    ("param", "csa_cmp_norm_w", "INPUT"),
    ("param", "csa_compress_state", "INOUT"),
    ("param", "csa_idx_wq_b", "INPUT"),
    ("param", "csa_idx_wq_b_scale", "INPUT"),
    ("param", "csa_weights_proj", "INPUT"),
    ("param", "csa_hadamard_idx", "INPUT"),
    ("param", "csa_inner_wkv", "INPUT"),
    ("param", "csa_inner_wgate", "INPUT"),
    ("param", "csa_inner_ape", "INPUT"),
    ("param", "csa_inner_norm_w", "INPUT"),
    ("param", "csa_inner_compress_state", "INOUT"),
    ("param", "cmp_kv", "INOUT"),
    ("param", "idx_kv_cache", "INOUT"),
    ("param", "idx_kv_scale", "INOUT"),
    ("param", "hc_ffn_fn", "INPUT"),
    ("param", "hc_ffn_scale", "INPUT"),
    ("param", "hc_ffn_base", "INPUT"),
    ("param", "norm_w", "INPUT"),
    ("param", "gate_w", "INPUT"),
    ("param", "gate_bias", "INPUT"),
    ("param", "tid2eid", "INPUT"),
    ("param", "routed_w1", "INPUT"),
    ("param", "routed_w1_scale", "INPUT"),
    ("param", "routed_w3", "INPUT"),
    ("param", "routed_w3_scale", "INPUT"),
    ("param", "routed_w2", "INPUT"),
    ("param", "routed_w2_scale", "INPUT"),
    ("param", "shared_w1", "INPUT"),
    ("param", "shared_w1_scale", "INPUT"),
    ("param", "shared_w3", "INPUT"),
    ("param", "shared_w3_scale", "INPUT"),
    ("param", "shared_w2", "INPUT"),
    ("param", "shared_w2_scale", "INPUT"),
    ("param", "freqs_cos", "INPUT"),
    ("param", "freqs_sin", "INPUT"),
    ("param", "block_table", "INPUT"),
    ("param", "position_ids", "INPUT"),
    ("param", "kv_seq_lens", "INPUT"),
    ("param", "hca_compress_state_block_table", "INPUT"),
    ("param", "csa_compress_state_block_table", "INPUT"),
    ("param", "csa_inner_compress_state_block_table", "INPUT"),
    ("param", "cmp_block_table", "INPUT"),
    ("param", "idx_block_table", "INPUT"),
    ("param", "block_counts", "INPUT"),
    ("param", "input_ids", "INPUT"),
    ("param", "hc_head_fn", "INPUT"),
    ("param", "hc_head_scale", "INPUT"),
    ("param", "hc_head_base", "INPUT"),
    ("param", "final_norm_w", "INPUT"),
    ("param", "lm_head_weight", "INPUT"),
    ("param", "logit_row_indices", "INPUT"),
    ("param", "pre_hc_hidden_out", "OUTPUT_EXISTING"),
    ("param", "hidden_out", "OUTPUT_EXISTING"),
    ("param", "logits", "OUTPUT_EXISTING"),
    ("param", "sampled_ids", "INOUT"),
...
# Ring sizing for the 43-layer graph, per runtime. The task window and dep pool
# match pypto-lib's daily CI sizing for this network (16384 / 16384). The heap is
# twice the compile-time default's 1 GiB: the MoE per-expert tile grid is static,
# so tile scratch is allocated for all 32 experts of every MoE layer whatever the
# routing turns out to be. host_build_graph commits its graph heap after
# orchestration at the size the graph needs and warns on a set ring_heap, so it
# takes none of these.
_RUNTIME_ENV = {
...
def _chip_spec(runtime: str, orchestration_source: str | Path | None) -> dict:
    """The ``decode_fwd`` chip entry: this case's orchestration plus its 368 incores.

    ``orchestration_source`` defaults to the AICPU orchestration next to this file;
    the host_build_graph case passes its Graph-form variant and keeps these incores.
    """
    source = Path(orchestration_source) if orchestration_source else HERE / "kernels/orchestration/decode_fwd.cpp"
    return {
        "name": "decode_fwd",
        "orchestration": {
            "source": str(source),
        ...
        "incores": [
            {
                "func_id": func_id,
                "name": name,
                "source": str(HERE / f"kernels/{core_type}/{name}.cpp"),
                "core_type": core_type,
                "signature": _sig(sig),
            }
...
def _allocate_params(worker: Worker, n_ranks: int) -> dict[str, Buffer]:
    """Allocate every per-rank parameter in child memory, once for the whole run.

    Returns ``{f"{name}_r{rank}": Buffer}``. A DEVICE_MALLOC buffer is private to
    its rank and is not auto-freed at end-of-task, so the same device memory backs
    every round and the runtime never mallocs, stages or copies back a parameter.
    """
    ...
    for step in _ARG_STEPS:
        ...
        for rank in range(n_ranks):
            buffers[f"{spec.name}_r{rank}"] = worker.alloc_child_tensor(
                worker_id=rank, shapes=spec.shape, dtype=getattr(DataType, spec.dtype)
            )
    return buffers


def _write_host_int32(handle: Buffer, values: list[int]) -> None:
    """Write ``values`` into a host ``Buffer``, leaving no live view behind.

    A memoryview exported from the shm blocks ``close()``, so the tensor over it
    is dropped before returning.
    """
    shm = handle.shm
    assert shm is not None
    view = torch.frombuffer(shm.buf, dtype=torch.int32, count=len(values))
    view.copy_(torch.tensor(values, dtype=torch.int32))
    del view
...
def _allocate_whole(worker: Worker, n_ranks: int) -> dict[str, Buffer]:
    ...
    orchestrator (host_build_graph) through ``get_tensor_data`` to size a task's
    block count. A child-memory tensor is passed to the device without a host
    view, so this one stays a host buffer.
    """
    buffers: dict[str, Buffer] = {}
    for step in _ARG_STEPS:
        if step[0] != "whole":
...
def _upload_fixture(worker: Worker, params: dict[str, Buffer], seed: int) -> None:
    """Stage the harvest's data into the child buffers, one parameter at a time.

    Each parameter is copied into its own born-shared staging buffer, pushed H2D,
    and released before the next is built, so host memory holds one parameter
    rather than the 42.6 GiB per-rank total. ``release_buffer`` (not ``close``)
    also drops the chip child's cached mapping of the staging segment.
    """
    for name, tensor in param_tensors(seed):
        target = params.get(name)
        if target is None:
            # num_tokens_per_owner: host-backed and already carrying the same value.
            continue
        ...
        try:
            shm = staging.shm
            assert shm is not None
            view = torch.frombuffer(shm.buf, dtype=tensor.dtype, count=tensor.numel()).view(tensor.shape)
            view.copy_(tensor)
            del view
            worker.copy_to(target, staging)
        finally:
            worker.release_buffer(staging)
...
def _build_config(
    ...
    *,
    ...
    config = CallConfig()
    runtime_env = _RUNTIME_ENV[runtime]
...
def _submit(orch, chip_handle, params, whole, config, n_ranks: int, keepalive: list) -> None:
    ...
    read by the C++ scheduler during the drain, which happens after this callback
    returns.
    """
    with orch.allocate_domain(
        ...
        for rank in range(n_ranks):
            ...
            for step in _ARG_STEPS:
                ...
                if step[0] == "param":
                    spec = PARAM_SPEC_BY_NAME[step[1]]
                    handle = params[f"{spec.name}_r{rank}"]
                    args.add_tensor(handle.tensor(spec.shape, getattr(DataType, spec.dtype)), arg_type)
                elif step[0] == "window":
                    args.add_tensor(
            ...
            orch.submit_next_level(chip_handle, args, config, worker=rank)


def run(  # noqa: PLR0913 -- one knob per CLI flag
    device_ids,
    platform: str,
    *,
    ...
    """Compile, then run ``rounds`` decode steps across ``device_ids``. 0 on success."""
    device_ids = [int(d) for d in device_ids]
    if not compile_only and len(device_ids) < N_RANKS:
        raise ValueError(f"deepseek_v4_flash_decode needs {N_RANKS} devices (EP world size), got {device_ids}")
    device_ids = device_ids[:N_RANKS]

    spec = _chip_spec(runtime, orchestration_source)
    # A literal module key, not __name__: running this file as a script and
    # importing it from the pytest wrapper must hit the same cache entry.
    cache_key = l3_compile_cache_key("examples.deepseek_v4_flash_decode", CASE_LABEL, spec["name"], platform, runtime)
    print(f"[dsv4] compiling {len(_KERNELS)} incores + orchestration for {platform}/{runtime}...", flush=True)
    if compile_workers:
        ...
        chip = compile_chip_callable_spec(spec, platform, runtime, cache_key)
    if compile_only:
        print("[dsv4] compile-only: done", flush=True)
        return 0

    # Multi-round diagnostics collide on the fixed artifact filenames under one
    # output prefix, so the scene-test rule holds here too: rounds > 1 keeps the
...
def parse_args(argv=None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--skip-golden",
        action="store_true",
        help="Skip the golden path: leave child memory uninitialized and upload nothing. "
        "Without it the harvest's 42.6 GiB-per-rank fixture is streamed into the same "
        "buffers; there is no golden to compare against either way",
    )
    parser.add_argument("--seed", type=int, default=1234, help="fixture seed, used unless --skip-golden")
    # The diagnostic flags mirror the scene-test CLI name for name, including the
    # optional-value forms: `--dump-args` alone is level 1, `--enable-pmu` alone is
    # PIPE_UTILIZATION(2).
    parser.add_argument(
        "--dump-args",
        nargs="?",
