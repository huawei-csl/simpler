 ...
 * inside SIMPLER_SCOPE(ScopeMode::MANUAL).
 */
...
__attribute__((visibility("default"))) void aicpu_orchestration_entry(const ChipTaskArgs &orch_args) {
    ...
    for (uint64_t b_idx = 0; b_idx < batch; b_idx++) {
        ...
        for (uint64_t q_idx = 0; q_idx < q_loop; q_idx++) {
            SIMPLER_SCOPE(ScopeMode::MANUAL) {
                ...
                simpler::tmr::Tensor qi = query.view(tile2d_shapes, qi_offsets);
                uint32_t out_view_offsets[2] = {static_cast<uint32_t>(cur_offset), 0};
                simpler::tmr::Tensor out_view = out.view(tile2d_shapes, out_view_offsets);
                ...
                for (uint64_t bn = 0; bn < bn_this_batch; bn++) {
                    ...
                    simpler::tmr::Tensor kj = key_cache.view(kv_shapes, kv_offsets);
                    simpler::tmr::Tensor vj = value_cache.view(kv_shapes, kv_offsets);
                    PROF_INC(prof_view_count, 2);
                    CYCLE_COUNT_LAP(prof_tensor_view);
                    ...
                    const simpler::tmr::Tensor &sij = qk_outs.get_ref(0);
                    PROF_INC(prof_submit_count, 1);
                    CYCLE_COUNT_LAP(prof_submit_task);

                    uint32_t sij_valid_shapes[2] = {static_cast<uint32_t>(q_tile), static_cast<uint32_t>(valid_len)};
                    uint32_t sij_valid_offsets[2] = {0, 0};
                    simpler::tmr::Tensor sij_valid = sij.view(sij_valid_shapes, sij_valid_offsets);
                    ...
                    const simpler::tmr::Tensor &pij_f16 = sf_outs.get_ref(0);
                    ...
                    const simpler::tmr::Tensor &oi_tmp = pv_outs.get_ref(0);
                    PROF_INC(prof_submit_count, 1);
                    CYCLE_COUNT_LAP(prof_submit_task);
    ...
    if (total > 0) {
        ...
        LOG_INFO("  scope            : %7.3fus (%5.1f%%)", cycles_to_us(prof_scope), prof_scope * 100.0 / total);
        LOG_INFO(
