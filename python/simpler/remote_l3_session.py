...
def _tensor_extent_bytes(tensor) -> int:
    ...
    if len(tensor.shapes) != len(tensor.strides):
        raise ValueError("remote TASK tensor shape and stride counts disagree")
    last_element = 0
    for shape, stride in zip(tensor.shapes, tensor.strides):  # noqa: B905
