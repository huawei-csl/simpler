...
static __aicore__ void softmax_prepare_n_impl(
    ...
    // --- pto::GlobalTensor types ---
    // 4D data views (1, 1, q_tile, n_blocks*block_size) for sij/pij.
    using GlobalDataMxN =
        pto::GlobalTensor<float, pto::Shape<1, BATCH, Q_LEN, M, N>, pto::Stride<1, M * N, M * N, N, 1>>;
    using GlobalDataMxN_bf16 =
        pto::GlobalTensor<bfloat16_t, pto::Shape<1, BATCH, Q_LEN, M, N>, pto::Stride<1, M * N, M * N, N, 1>>;
    ...
    using GlobalScalarND =
        pto::GlobalTensor<float, pto::Shape<1, 1, 1, kScalarRows, kScalarCols>, pto::Stride<1, 1, 1, kScalarCols, 1>>;
    ...
    using TileScalarND = pto::Tile<
        pto::TileType::Vec, float, kScalarRows, kScalarCols, pto::BLayout::RowMajor, kScalarRows, kScalarCols>;
    // RowMajor (1, M) tile for element-wise arithmetic via TRESHAPE
    using TileScalarRow = pto::Tile<pto::TileType::Vec, float, 1, M, pto::BLayout::RowMajor, 1, M>;
