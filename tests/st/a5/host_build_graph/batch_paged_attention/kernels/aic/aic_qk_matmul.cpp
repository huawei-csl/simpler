...
static __aicore__ void qk_matmul_batch_impl(
    ...
    using GlobalA = pto::GlobalTensor<bfloat16_t, pto::Shape<1, 1, 1, M, K>, pto::Stride<M * K, M * K, M * K, K, 1>>;
    using GlobalB = pto::GlobalTensor<
        bfloat16_t, pto::Shape<1, 1, 1, K, N>, pto::Stride<K * N, K * N, K * N, 1, K>, pto::Layout::DN>;
    using GlobalOut = pto::GlobalTensor<float, pto::Shape<1, 1, 1, M, N>, pto::Stride<M * N, M * N, M * N, N, 1>>;

    using TileMatA =
        pto::Tile<pto::TileType::Mat, bfloat16_t, M, K, pto::BLayout::ColMajor, M, K, pto::SLayout::RowMajor, 512>;
    using TileMatB =
        pto::Tile<pto::TileType::Mat, bfloat16_t, K, N, pto::BLayout::RowMajor, K, N, pto::SLayout::ColMajor, 512>;

    using LeftTile = pto::TileLeft<bfloat16_t, M, K, M, K>;
    using RightTile = pto::TileRight<bfloat16_t, K, N, K, N>;
    using AccTile = pto::TileAcc<float, M, N, M, N>;
