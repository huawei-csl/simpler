...
__attribute__((visibility("default"))) void aicpu_orchestration_entry(const ChipTaskArgs &orch_args) {
    ...
    simpler::hbg::Tensor out = simpler::hbg::make_tensor_external(out_ptr, out_shapes, 2, DataType::FLOAT32, true);
    ...
    for (uint64_t q_idx = 0; q_idx < q_loop; q_idx++) {
        ...
        for (uint64_t chunk_idx = 0; chunk_idx < num_chunks; chunk_idx++) {
            ...
            SIMPLER_SCOPE() {
                ...
                const simpler::hbg::Tensor &oi_batch = alloc_outs.get_ref(0);
                const simpler::hbg::Tensor &li_batch = alloc_outs.get_ref(1);
                const simpler::hbg::Tensor &mi_batch = alloc_outs.get_ref(2);
                ...
                for (uint64_t bn = 0; bn < max_bn; bn++) {
                    SIMPLER_SCOPE() {
                        ...
                        const simpler::hbg::Tensor &sij_b = qk_outs.get_ref(0);
                        ...
                        const simpler::hbg::Tensor &pij_b = sf_outs.get_ref(0);
                        const simpler::hbg::Tensor &mij_b = sf_outs.get_ref(1);
                        const simpler::hbg::Tensor &lij_b = sf_outs.get_ref(2);
                        ...
                        const simpler::hbg::Tensor &oi_new_b = pv_outs.get_ref(0);
