...
__attribute__((visibility("default"))) void aicpu_orchestration_entry(const ChipTaskArgs &orch_args) {
    ...
    const simpler::hbg::Tensor &ext_X = orch_args.tensor(3).ref();
    const simpler::hbg::Tensor &ext_Y = orch_args.tensor(4).ref();
    ...
    for (int group_idx = 0; group_idx < max_groups; group_idx++) {
        if (group_idx < num_matmul_groups) {
            ...
            simpler::hbg::Tensor A_view = ext_A.view(matmul_group_shapes, view_offsets);
            simpler::hbg::Tensor B_view = ext_B.view(matmul_group_shapes, view_offsets);
            simpler::hbg::Tensor C_view = ext_C.view(matmul_group_shapes, view_offsets);
        ...
        if (group_idx < num_add_groups) {
            ...
            simpler::hbg::Tensor X_view = ext_X.view(add_group_shapes, view_offsets);
            simpler::hbg::Tensor Y_view = ext_Y.view(add_group_shapes, view_offsets);
            simpler::hbg::Tensor Z_view = ext_Z.view(add_group_shapes, view_offsets);
