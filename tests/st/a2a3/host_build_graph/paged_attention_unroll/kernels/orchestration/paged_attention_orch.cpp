 ...
 * slot is a materialized simpler::hbg::Tensor; the Arg carries no TensorCreateInfo (the
 ...
 *            4 out (output buffer the update task writes — add_output(simpler::hbg::Tensor))
...
static void process_qtile_scope(const CoreTaskArgs &ctx) {
    const simpler::hbg::Tensor &query = ctx.tensor(0).ref();
    const simpler::hbg::Tensor &key_cache = ctx.tensor(1).ref();
    ...
    simpler::hbg::Tensor qi = query.view(qi_shapes, qi_offsets);
    ...
    simpler::hbg::Tensor out_view = out.view(out_view_shapes, out_view_offsets, true);
#ifdef ENABLE_PROFILING
    g_prof.view_count += 2;
    ...
    for (uint64_t bn = 0; bn < bn_this_batch; bn += N_UNROLL) {
        ...
        const simpler::hbg::Tensor &sij_buf = qk_outs.get_ref(0);
#ifdef ENABLE_PROFILING
        g_prof.submit_count++;
        CYCLE_COUNT_LAP(g_prof.submit_task);
#endif
        ...
        const simpler::hbg::Tensor &pij_buf = sf_outs.get_ref(0);
        ...
        const simpler::hbg::Tensor &oi_new = pv_outs.get_ref(0);
#ifdef ENABLE_PROFILING
        g_prof.submit_count++;
        CYCLE_COUNT_LAP(g_prof.submit_task);
#endif
...
__attribute__((visibility("default"))) void aicpu_orchestration_entry(const ChipTaskArgs &orch_args) {
    ...
    simpler::hbg::Tensor query = simpler::hbg::make_tensor_external(query_ptr, query_shapes, 2, data_type, false);
    simpler::hbg::Tensor key_cache = simpler::hbg::make_tensor_external(kc_ptr, key_cache_shapes, 2, data_type, false);
    simpler::hbg::Tensor value_cache =
        simpler::hbg::make_tensor_external(vc_ptr, value_cache_shapes, 2, data_type, false);
