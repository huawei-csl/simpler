...
# -----------------------------------------------------------------------------------------------------------
"""core_swimlane — generate an AICore intra-core swimlane trace.json for a task.
...
# which the orchestration builds per dispatch. core_swimlane runs an isolated
...
def load_kernel_meta(test_path: Path, func_id: int, platform: str):
    spec = importlib.util.spec_from_file_location("core_swimlane_testmod", str(test_path))
...
def get_or_run_dump(
    ...
    if variant != "sim":
        if device:
            ...
            print(
                "[core_swimlane] WARNING: onboard dump with no locked device — not "
    ...
    print(f"[core_swimlane] running dump: {' '.join(cmd[1:])}")
        ...
        for slot in restore_slots:
            ...
            if bin_size == 0:
                raise ValueError(
                    ...
                    "CoreTaskArgs::dump(...) in orchestration and capture a fresh hybrid level-3 dump"
                )
...
def emit_replay_host(tensor_count: int, args, block_num: int = 1) -> str:
    ...
    return f"""\
// Auto-generated by simpler_setup.tools.core_swimlane — do not edit by hand.
...
project(core_swimlane_{name}_replay)
...
def smoke_build(ws: Path, env, cfg, pto_isa_root: str):
    ...
    print("[core_swimlane] smoke build OK (replay_entry, launch_replay present)")
...
def collect(ws: Path, env, max_time: int, device=None, dest_name: str = "trace.json"):
    if device:
        ...
        print(
            "[core_swimlane] WARNING: task-submit not found; running run_collect.sh "
...
def apply_arg_overrides(kargs: list[dict], set_arg, ap: argparse.ArgumentParser):
    ...
    for slot, value in set_arg:
        ...
        if a["kind"] == "scalar":
            ...
            print(f"[core_swimlane] overriding scalar slot {slot}: {old} -> {value}{note}")
        else:
            ...
            a["fill"] = value
            print(
                f"[core_swimlane] filling tensor slot {slot} ({a['dtype']} {a['shape']}) with {value} in every element"
            )
...
def main():
    ...
    if not args.case and meta["auto_case"]:
        print(f"[core_swimlane] no --case; auto-pinned first {args.platform} case: {meta['auto_case']}")
    ...
    print(f"[core_swimlane] manifest: {manifest}")
    ...
    print(
        f"[core_swimlane] func_id={func_id_list} task={chosen} mix={mix_func_ids} mode={mode} "
        f"block_num={block_num}\n              members=[{member_desc}]"
    )

    scalars = [a for a in kargs if a["kind"] == "scalar"]
    print(f"[core_swimlane] task {chosen}: {tensor_count} tensors, {len(scalars)} scalars")
    ...
    ws = PROJECT_ROOT / "outputs" / f"core_swimlane_{label}_{ts}"
    ...
    if args.no_collect:
        print(f"[core_swimlane] --no-collect: stopping after smoke build.\n  {ws}")
        return
    trace, trace_perfetto = collect(ws, env, args.max_time, device, dest_name=f"{label}_trace.json")
    print(f"[core_swimlane] DONE. trace.json (Insight) -> {trace}")
    if trace_perfetto:
        print(f"[core_swimlane]       perfetto-friendly  -> {trace_perfetto}")
