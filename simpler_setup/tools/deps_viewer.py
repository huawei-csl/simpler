...
``--edge-mode`` selects which edges are visible. Reduction is skipped with a
...
def _transitive_reduction_dataflow(edges, nodes, annotations, tensor_table, task_table):
    """Reduce structural candidates only after full-annotation dataflow proof.

    Structural reduction only selects candidates. Every byte-level proof below
    reads the complete original annotations, before any display filtering. A
    candidate creator edge is removable only when every creator annotation on
    that pair is an exactly-known INOUT region, and every byte has direct
    TensorMap dataflow from an earlier Output and to a later INOUT owned by the
    same creator. OUTPUT_EXISTING can begin a reuse generation, so it is always
    preserved. Missing or ambiguous evidence also preserves the edge; graph
    reachability alone never joins reuse rounds.
    """
...
Examples:
  ...
  %(prog)s deps.json --edge-mode reduced_dataflow  # require direct INOUT dataflow before omitting creator edges
  %(prog)s deps.json --edge-mode omitted_dataflow  # show only dataflow-verified redundant edges
""",
    )
...
def _apply_edge_mode(edge_mode, output_format, edges, nodes, annotations, tensor_table, task_table):
    ...
    is_dataflow = edge_mode.endswith("_dataflow")
    if is_dataflow:
        kept, removed, is_dag = _transitive_reduction_dataflow(edges, nodes, annotations, tensor_table, task_table)
    else:
