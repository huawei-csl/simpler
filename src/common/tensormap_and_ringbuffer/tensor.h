 ...
 */
/**
 * The `tmr` runtime's working tensor type. See the struct comment below for how it
 * relates to the boundary `ChipTensor` in src/common/task_interface/tensor.h.
 */
...
/**
 * The runtime's working tensor (128B = 2 cache lines).
 *
 * A `ChipTensor` names a resolved buffer and a strided view of it; this type adds
 * what the runtime has *decided* about that view — its producing task, its
 * overlap-detection version and dependency treatment, and two caches derived from
 * the geometry. The orchestration entry receives these, never a bare `ChipTensor`:
 * the host converts each boundary argument once, before orchestration runs.
 *
...
struct alignas(64) Tensor {
    ...
    // from_boundary() / make_tensor_external() / init_external() / a view op.
    Tensor() = default;

    // --- Copy / move / destroy ---
    // Trivially copyable: the payload is memcpy'd to the device, so the default
    // copy must be byte-for-byte. `init_from` is the copy that recomputes the
    // derived pair from geometry instead of reading it across.
    ...
    // operates on a Tensor& through its public members. Kept out of the
    // wire/host-facing Tensor so this header has no dependency on the
    ...
    Tensor view(const uint32_t view_shapes[], const uint32_t view_offsets[], bool in_manual_dep = false) const {
        Tensor result;
        result.init_geometry_from(*this);
        bool empty = false;
        for (uint32_t i = 0; i < ndims; i++) {
            if (view_shapes[i] == 0) {
                empty = true;
    ...
    Tensor transpose(uint32_t x, uint32_t y, bool in_manual_dep = false) const {
        ...
        Tensor result;
        result.init_geometry_from(*this);
        // init_geometry_from leaves the derived pair alone, so a view op that keeps
        // the source's stride has to bring it forward itself.
        for (uint32_t i = 0; i < ndims; i++)
            result.strides[i] = strides[i];
    ...
    Tensor slice(uint32_t dim, uint32_t start, uint32_t end, uint32_t step = 1, bool in_manual_dep = false) const {
        ...
        Tensor result;
        result.init_geometry_from(*this);
        // init_geometry_from leaves the derived pair alone; carry the source's
        // stride forward before patching the sliced dim.
        for (uint32_t i = 0; i < ndims; i++)
            result.strides[i] = strides[i];
    ...
    /// Adopt a boundary `ChipTensor`: its geometry is taken as-is and the
    /// runtime-decided fields start at their unset values. An argument arriving
    /// from outside names no producing task, so `owner_task_id` is invalid and
    /// dependency inference treats it as external.
    static Tensor from_boundary(const ChipTensor &arg) {
        Tensor t{};
        t.buffer = arg.buffer;
        t.start_offset = arg.start_offset;
        t.ndims = arg.ndims;
        t.dtype = arg.dtype;
        t.address_space = arg.address_space;
        for (uint32_t i = 0; i < MAX_TENSOR_DIMS; i++) {
            t.shapes[i] = arg.shapes[i];
            t.strides[i] = arg.strides[i];
        }
    ...
    ChipTensor to_boundary() const {
        ChipTensor arg{};
        arg.buffer = buffer;
        arg.start_offset = start_offset;
        arg.ndims = ndims;
        arg.dtype = dtype;
        arg.address_space = address_space;
        for (uint32_t i = 0; i < MAX_TENSOR_DIMS; i++) {
            arg.shapes[i] = shapes[i];
            arg.strides[i] = strides[i];
        }
        return arg;
    }
...
static_assert(
    std::is_trivially_copyable_v<Tensor> && std::is_standard_layout_v<Tensor>,
    "the payload holding these is copied to the device verbatim"
);
