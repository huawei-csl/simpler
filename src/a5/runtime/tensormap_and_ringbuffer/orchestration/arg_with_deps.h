 ...
 *
 * Layering:
 *   - Primitive:   Arg + set_dependencies(ptr, count) in types.h.
 ...
 *
 * This file is auto-included at the bottom of orchestration_api.h so
 * orchestration sources see CoreTaskArgsWithDeps after a single `#include
 * "orchestration_api.h"`. The split is purely organizational —
...
#include "orchestration_api.h"  // Arg, MixedKernels, rt_submit_* primitives

template <size_t MAX_DEP_COUNT = 16>
class CoreTaskArgsWithDeps : private CoreTaskArgs {
public:
    // simpler::tmr::Tensor / scalar setters — forward to Arg
