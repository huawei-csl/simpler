...
struct AsyncWaitList {
            ...
            if (msg.kind == MSG_KIND_CONDITION) {
                ...
                if (entry == nullptr) {
                    ...
                    if (count >= MAX_ASYNC_WAITS) {
                        error_code = SIMPLER_ERROR_ASYNC_WAIT_OVERFLOW;
                        return drained;
                    }
                    entry = &entries[count++];
                ...
                error_code = SIMPLER_ERROR_ASYNC_REGISTRATION_FAILED;
                return drained;
            }
        }
        return drained;
    ...
    AsyncPollResult poll_and_complete(AICoreCompletionMailbox *aicore_mailbox, PTO2SchedulerState *sched, int thread_idx);
};
