/*
 * Frozen at 324e90a7 and renamed.
 *
 * Both tensor types this runtime deals in: the boundary `ChipTensor` an argument
 * arrives as (src/common/task_interface/tensor.h), and `Tensor`, the internal form
 * carrying the scheduling fields -- version, manual_dep, owner_task_id,
 * extent_elem_cache. Before main split them per runtime these were one type named
 * ChipTensor; this is that type, renamed so the boundary one can coexist.
 *
 * Not host_build_graph's `simpler::hbg::Tensor`: that one embeds hbg's id-space
 * TaskId, and this runtime's TaskId encodes a ring slot (see task_id.h). The two
 * are not interchangeable in a shared-memory image.
 */
#pragma once
...
public:
    // Conversions to and from the boundary type. The pre-split form needed none
    // -- it WAS the boundary type -- so these mirror host_build_graph's: geometry
    // and resolved address cross, scheduling fields do not. A tensor arriving from
    // outside names no producing task, so owner_task_id is invalid and dependency
    // inference treats it as external.
...
static_assert(
    std::is_trivially_copyable_v<Tensor>, "Tensor must be trivially copyable for DMA / wire transport"
);
...
static_assert(offsetof(Tensor, address_space) == 43, "address_space must be at byte 43 (cacheline 1)");
