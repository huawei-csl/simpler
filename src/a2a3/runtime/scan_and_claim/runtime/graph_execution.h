...
// Wire representation of Tensor. Tensor itself is a host/runtime C++ type with
...
struct GraphPredicate {
    ...
    // Fixed at record time: a Graph with a variable Tensor shape is rejected
...
inline GraphTensor graph_tensor_pack(const Tensor &tensor) {
    GraphTensor packed{};
...
inline void graph_tensor_unpack(const GraphTensor &packed, Tensor *tensor) {
...
struct GraphExecution {
    ...
    Tensor *node_tensor_pool{nullptr};
...
static_assert(
    alignof(GraphNodeStorage) % alignof(Tensor) == 0,
    "a node entry must be at least Tensor-aligned: the tensor pool follows the node array"
);
static_assert(sizeof(Tensor) % alignof(uint64_t) == 0, "the tensor stride must keep the scalar pool aligned");
...
static_assert(sizeof(GraphTensor) <= sizeof(Tensor));
...
inline constexpr size_t graph_boundary_tensor_pool_slots(uint32_t tensor_count) {
    ...
    return (bytes + sizeof(Tensor) - 1) / sizeof(Tensor);
}

// The outer GRAPH task's heap tail occupies
// [GraphExecution][GraphNodeStorage x node_count][Tensor x tensor_arg_count]
...
inline bool graph_execution_storage_layout(
    ...
    out->scalars_offset = out->tensors_offset + static_cast<size_t>(tensor_arg_count) * sizeof(Tensor);
