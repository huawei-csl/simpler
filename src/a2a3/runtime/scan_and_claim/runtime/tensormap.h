...
struct alignas(64) PTO2TensorMapEntry {
    ...
    PTO2TensorMapEntry
        *next_in_bucket;      // 8B [8, 16):  next entry in hash bucket chain (overlays Tensor::buffer.size)
    TaskId producer_task_id;  // 8B [16,24):  mirrors Tensor::owner_task_id slot
...
struct PTO2TensorMap {
     ...
     */
    void insert(const Tensor &tensor, TaskId producer_task_id) {
