...
LoopAction SchedulerContext::handle_orchestrator_exit(PTO2SharedMemoryHeader *header, Runtime *runtime, int32_t &task_count)
{
...
void format_core_status(char *buf, size_t buf_size, int32_t core_id, bool idle, const CoreExecState *core_state, uint64_t reg_addr_for_cond)
{
    if (idle)
    {
...
void SchedulerContext::log_stall_diagnostics(int32_t thread_idx, [[maybe_unused]] int32_t task_count)
{
...
void SchedulerContext::log_shutdown_stall_snapshot([[maybe_unused]] int32_t trigger_idle_iterations, [[maybe_unused]] int32_t trigger_last_progress_count)
{
...
int32_t SchedulerContext::handle_timeout_exit(int32_t thread_idx, PTO2SharedMemoryHeader *header, Runtime *runtime, int32_t idle_iterations, int32_t last_progress_count)
{
