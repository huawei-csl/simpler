...
static void orch_report_fatal_v(PTO2OrchestratorState *orch, int32_t error_code, const char *fmt, va_list args)
{
    ...
    if (latched_code != PTO2_ERROR_NONE && latched_code != error_code) return;
}
...
static bool append_fanin_or_fail(PTO2OrchestratorState *orch, PTO2TaskSlotState *prod_state, PTO2FaninBuilder *fanin_builder, uint8_t ring_id)
{
...
static bool prepare_task(PTO2OrchestratorState *orch, const Arg &args, int32_t total_output_size, ActiveMask active_mask, PTO2PreparedTask *out)
{
...
static TaskOutputTensors submit_task_common(PTO2OrchestratorState *orch, const Arg &args, ActiveMask active_mask, int32_t aic_kernel_id, int32_t aiv0_kernel_id, int32_t aiv1_kernel_id)
{
