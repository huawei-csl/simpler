...
struct PTO2TaskPayload {
    ...
    // Real producer-edge count, the early-dispatch target (without the wiring
    // +1 that fanin_count/ready_fanin carry). A consumer whose dispatch_fanin
    // reaches fanin_actual_count has every producer flagged-and-fully-published
    // or pre-completed => it is an early-dispatch candidate.
    ...
    // === Early-dispatch metadata (AICPU-side only) ===
    ...
    // Early-dispatch CANDIDATE detection (event-driven, dual of the ready fanin):
    ...
    // dispatch_fanin == fanin_actual_count => early-dispatch candidate.
    ...
    // Lock-free claim state shared by the stagers and the completion-path release:
    // 0=NONE, 1=STAGING (stable gated), 3=DISPATCHED (2=STAGED unused). STAGING
    // holds while many threads stage blocks concurrently, each claiming via the
    // atomic next_block_idx and OR-ing its cores into staged_core_mask.
    ...
    // sync_start rendezvous: count of this task's gated CORES currently occupying a
    // RUNNING slot. Doorbells ring only once this reaches popcount(staged_core_mask)
    // AND the producer released, so all cores launch atomically. Counted per-core
    // (shape-agnostic; a MIX block spans a cluster). Unused (0) for non-sync_start.
    ...
    // reinitialization; only cancellation clears OWNER during a task lifetime.
    std::atomic<uint8_t> early_sync_drain_state{PTO2_EARLY_SYNC_DRAIN_NONE};
    // Dispatch predicate (AICPU-only), cache-line aligned at its natural offset.
    // With PTO2_MAX_FANIN=128 the inline wireless fanin arrays exceed upstream's
    // fixed 576/640 metadata budget, so this payload does NOT preserve upstream's
    // predicate@576 / tensors@640 offsets. That AICore fixed-offset arg-
    // materialization contract only matters on the early-dispatch gated path;
    // it is re-established (e.g. by moving fanin to a side array) when early
    // dispatch is wired. Normal dispatch reads tensors via struct members only.
    alignas(64) DispatchPredicate predicate;
    // === Tensors (alignas(64) forces alignment) ===
    ...
    void init(const L0TaskArgs &args, TaskOutputTensors &result, PTO2TaskAllocResult &alloc_result, PTO2OutputLayout &layout) {
        ...
        // Early-dispatch metadata — the single init point for these fields.
        // early_dispatch_state / staged_core_mask / dispatch_fanin are CONSUMER-side:
        // a task whose own allow_early_resolve is false still has them touched when
        // one of ITS producers is flagged (propagate_dispatch_fanin bumps
        // dispatch_fanin and may CAS early_dispatch_state on any consumer). So they
        // MUST be zeroed here unconditionally, once per submit.
...
// Fused wireless-fanin + early-dispatch layout: flat fanin arrays (sized by
// PTO2_MAX_FANIN) + early-dispatch metadata + predicate, then bulk tensor/scalar
// data. Offsets are relative (not the upstream fixed 576/640): with
...
static_assert(
    sizeof(PTO2TaskPayload) == offsetof(PTO2TaskPayload, scalars) + MAX_SCALAR_ARGS * sizeof(uint64_t),
