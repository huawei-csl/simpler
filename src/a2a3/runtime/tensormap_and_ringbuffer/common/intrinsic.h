...
struct GlobalContext {
    ...
    // Address of this core's slice of the AICore TraCR record region, or 0 when
    // TraCR is off. The scheduler slices the region by worker id during cold
    // start, which is the same index AICore's kernel entry publishes its
    // identity word under -- the equivalence
    // chip_swimlane_aicore_rotation_table[block_idx] already relies on.
    //
    // A kernel generated by PyPTO links into a self-contained AICore image and
    // so cannot call the platform's get_tracr_aicore_buffer(); it seeds its own
    // block-local slot from this field instead. Read via
    // get_tracr_aicore_slice(args).
    uint64_t tracr_aicore_slice;
};
...
/**
 * Return this core's slice of the AICore TraCR record region, or nullptr when
 * TraCR is off. Runtime-provided, like get_dma_workspace: the host allocates
 * the region once per device and the scheduler injects each core's slice into
 * its context, so a kernel does not thread it as a user arg.
 *
 * A generated kernel needs this because it links into a self-contained AICore
 * image and cannot reach the platform's get_tracr_aicore_buffer(); it seeds its
 * own block-local slot from here at kernel entry. A platform-resident kernel
 * keeps using the platform accessor and never calls this.
 */
static __aicore__ inline __gm__ int64_t *get_tracr_aicore_slice(__gm__ int64_t *args) {
    __gm__ GlobalContext *ctx =
        reinterpret_cast<__gm__ GlobalContext *>(static_cast<uint64_t>(args[SPMD_GLOBAL_CONTEXT_INDEX]));
    return reinterpret_cast<__gm__ int64_t *>(ctx->tracr_aicore_slice);
}
