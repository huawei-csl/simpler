...
void SchedulerContext::check_running_cores_for_completion(
...
#endif
#ifdef __SIMULATED_DEVICE__
    // One read of this queue's status register retires everything its contiguous
    // prefix covers, plus the positions that finished ahead of it. That is the
    // whole point of the GroupQueue: the manager stops asking each core in turn,
    // so a pass costs one local read rather than one serialised MMIO load per core.
    {
    ...
    while (running_core_states.has_value()) {
        ...
        // Unreachable under the simulated device: the queue-driven retire above
        // returns before this sweep, because a GroupQueue reports positions rather
        // than per-core COND values.
...
#ifdef __SIMULATED_DEVICE__
            // Retries are unbounded by design, so a drain that can never be satisfied
            // shows up only as a spin. Report the first time, and thereafter only when
            // the shortfall changes, so a stuck drain names its own numbers without
            // flooding the device log.
            {
                static int32_t last_avail = -12345;
                static int32_t last_block = -12345;
                static uint32_t reports = 0;
                if ((available != last_avail || block_num != last_block) && reports < 32) {
                    last_avail = available;
                    last_block = block_num;
                    ++reports;
                    LOG_ERROR(
                        "[GQ_DRAIN] insufficient: available=%d block_num=%d shape=%d gated=%d",
                        available, block_num, static_cast<int32_t>(shape), gated ? 1 : 0
                    );
                }
            }
#endif
