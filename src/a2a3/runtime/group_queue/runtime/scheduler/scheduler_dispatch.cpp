...
// byte offsets in dispatch_payload.h (the AICore .o cannot see TaskPayload).
...
void capture_shared_ready_depth(SchedulerState *scheduler, int16_t shared_depth[CHIP_SWIMLANE_NUM_QUEUE_SHAPES]) {
    ...
    for (int shape = 0; shape < CHIP_SWIMLANE_NUM_QUEUE_SHAPES; shape++) {
        const size_t depth = scheduler->ready_depth(shape) + scheduler->ready_sync_queues[shape].size();
...
static_assert(
    RUNTIME_MAX_WORKER <= EARLY_DISPATCH_CORE_MASK_WORDS * 64, "staged_core_mask too small for RUNTIME_MAX_WORKER cores"
);
...
SchedulerContext::PublishHandle SchedulerContext::prepare_subtask_to_core(
    ...
    return PublishHandle{
        ...
        slot_state.to_descriptor().task_id.local_id(),
        &slot_state,
        subslot,
...
void SchedulerContext::dispatch_shape(
    ...
    while (cores.has_value() && !entered_drain) {
        ...
        if (gq_group::ENABLED) {
            ...
            // what this pass may offer just as the free cores do. Sizing here is what
            // keeps a submit from ever being refused: the manager never offers work
            // the controller has nowhere to put.
            const asimgq::SimQueueStatus st = asimgq::read_queue_status(static_cast<uint32_t>(thread_idx));
            const int hold_room = static_cast<int>(st.held_room / SUBTASK_SLOT_COUNT);
            if (hold_room < want) want = hold_room;
            if (want <= 0) break;
        }
#endif
...
void SchedulerContext::dispatch_ready_tasks(
    ...
    run_staging_order(
        ...
        [&](ResourceShape shape, CoreTracker::DispatchPhase phase) {
            dispatch_shape(
                thread_idx, sched_->ready_for(thread_idx), shape, phase, tracker, entered_drain, made_progress,
                try_pushed
            );
            return entered_drain;
        },
...
int32_t SchedulerContext::resolve_and_dispatch(Runtime *runtime, int32_t thread_idx) {
    ...
    while (true) {
        ...
        // Refill from the ready group queue before looking for work: a thread takes
        // whole groups, so its rows are either carrying one or ready for the next.
        // This thread's position room is the bound -- a group consumes GROUP_SIZE.
