...
#include <atomic>
#include "common/platform_config.h"  // PLATFORM_MAX_AICPU_THREADS, sizing the per-owner ready rows
#include "scheduler/task_grouping.h"
 ...
 */
// Where this manager put each task it submitted, by local id. A consumer names its
// in-group producers by the positions their queue assigned, and the grouping
// contract guarantees those producers are on the same queue -- so one array,
// private to the manager, answers it without touching shared state.
//
// Sized like the ready stamps and reset beside them: a run restarts every queue's
// position space, so a position recorded in an earlier round names nothing in this
// one.
...
struct SchedulerState {
    ...
    // Under the grouping contract scheduling *is* partitioned, by group rather
    // than by task id: a group belongs to the thread that claimed it, so its
    // members must reach that thread's dispatch loop and no other. These rows
    // carry them. Each is wired to a disjoint span of the same per-shape slot
    // array ready_queues[] uses, so the partition costs no extra arena; the
    // unpartitioned queue above is then unused and stays empty.
    ChipReadyQueue ready_queues_gq[PLATFORM_MAX_AICPU_THREADS][NUM_RESOURCE_SHAPES];
    ...
    // Producers of `local_id` that have not retired, as positions in this manager's
    // queue. Returns the count, or -1 when the task must take the ordinary path:
    // an unmet producer outside its group, or more of them than one entry can name.
    //
    // Under the grouping contract an unmet producer is a group member by
    // construction, so -1 means the annotation and the graph disagree -- a
    // build-time defect rather than a case to absorb at run time.
    // >=0: that many unmet in-group producers, named in `out`.
    //  -1: an edge the controller cannot be told about (producer outside the group,
    //      or not yet submitted) -- the caller must not submit this task.
    //  -2: more unmet producers than the controller has comparators for; the caller
    //      must hold the task until enough of them retire.
    static constexpr int kDepsUnexpressable = -1;
    ...
    int group_deps(ChipTaskSlotState *s, int32_t local_id, uint64_t *out, uint32_t max_out) const {
        ...
        for (int32_t k = 0; k < p.fanin_count; ++k) {
            ...
            if (n >= max_out) {
                g_gq_deps_toomany.fetch_add(1, std::memory_order_relaxed);
                return kDepsTooMany;
            }
            out[n++] = pos;
        }
        if (n > 0) {
            g_gq_deps_named.fetch_add(1, std::memory_order_relaxed);
        } else {
            g_gq_deps_allcomplete.fetch_add(1, std::memory_order_relaxed);
        }
    ...
    int classify_fanin_state(ChipTaskSlotState *s) const {
        ...
        for (int32_t i = start; i >= 0; i--) {
            ...
            // its cores. It only counts as resolvable once it has a position to
            ...
            if (gq_group::active() && !tasks.is_completed(fanin[i]) && self_run.holds(fanin[i])) {
                ...
                if (!claimed) g_gq_skip_unclaimed.fetch_add(1, std::memory_order_relaxed);
                if (!positioned) g_gq_skip_nopos.fetch_add(1, std::memory_order_relaxed);
            }
